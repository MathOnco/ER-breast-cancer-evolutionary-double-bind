...
filename = './MCF7_Sens&PacliR.xlsx';

%% fitting process (MCF7, Doxo):
% read in 100% conditions only:
cell_counts = xlsread(filename,1,'E2:K2')
YFPdata = xlsread(filename,1,'E3:K5')
RFPdata = xlsread(filename,1,'M18:S20')
[n,~]=size(YFPdata);
...
color_indices = xlsread(filename,1,'T3:T20');
cell_counts = xlsread(filename,1,'E2:K2');
YFP = xlsread(filename,1,'E3:K17');
RFP = xlsread(filename,1,'M6:S20');
[n,~]=size(YFP);
...
% printPNG(figure(1),'ValidationPlots/T47D_PacliR.png');
